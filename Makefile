...
DEVKIT_IMG ?= ghcr.io/jkoelker/cotton:latest-devkit
DEVKIT_PHONY_FILE ?= $(REPO_ROOT)/.docker-$(subst :,.,$(subst /,-,$(DEVKIT_IMG)))
...
# Rebuild the devkit image on any changes to Dockerfile.devkit or tools.
$(DEVKIT_PHONY_FILE): $(REPO_ROOT)/tools/go.mod
$(DEVKIT_PHONY_FILE): $(REPO_ROOT)/tools/go.sum
$(DEVKIT_PHONY_FILE): $(REPO_ROOT)/tools/tools.go
